...
static const char *TAG = "[jsapi_coercetoobject]";

napi_value testNapiCoerceToObject(napi_env env, napi_callback_info info)
{
    // pages/javascript/jsabstractops/napicoercetoobject
    ...
    // Call napi_coerce_to_object(), any -> object
    status = napi_coerce_to_object(env, args[0], &result);
    if (status != napi_ok) {
        ...
        if (status == napi_ok && extended_error_info != NULL) {
            ...
            std::string res = "Failed to coerce to object. em = " + std::string(errorMessage) +
