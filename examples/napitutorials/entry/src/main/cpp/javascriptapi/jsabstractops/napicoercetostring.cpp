...
static const char *TAG = "[jsapi_coercetostring]";

napi_value testNapiCoerceToString(napi_env env, napi_callback_info info)
{
    // pages/javascript/jsabstractops/napicoercetostring
    ...
    // Call napi_coerce_to_string(), any -> string
    status = napi_coerce_to_string(env, args[0], &result);
    if (status != napi_ok) {
        ...
        if (status == napi_ok && extended_error_info != NULL) {
            ...
            std::string res = "Failed to coerce to string. em = " + std::string(errorMessage) +
