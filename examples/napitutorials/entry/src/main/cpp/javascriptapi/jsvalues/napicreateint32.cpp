...
napi_value testNapiCreateInt32(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        getErrMsg(status, env, extended_error_info, "Failed to convert argument to int32", TAG);
        return NULL;
    }
    ...
    status = napi_create_int32(env, intValue, &result);
    if (status != napi_ok) {
        getErrMsg(status, env, extended_error_info, "Failed to create int32 value", TAG);
        return NULL;
    }
