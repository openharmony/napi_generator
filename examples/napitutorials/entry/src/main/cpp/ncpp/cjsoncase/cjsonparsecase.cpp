...
napi_value KH418_CJSON_Parse(napi_env env, napi_callback_info info)
{
     ...
     */
    status = napi_typeof(env, args[0], &valuetypevalue);
    if (status != napi_ok) {
    ...
    status = napi_get_value_string_utf8(env, args[0], NULL, 0, &strSize0);
    if (status != napi_ok) {
    ...
    /* [NAPI_GEN]: 用于获取字符串*/
    status = napi_get_value_string_utf8(env, args[0], valueIn, strSize0 + 1, &strSize0);
    if (status != napi_ok) {
        getErrMsg(status, env, extended_error_info, "get value string", tag);
        delete[] valueIn;
        return nullptr;
     ...
     */
    status = napi_create_string_utf8(env, json == nullptr? "": (json->valuestring == nullptr? "": json->valuestring), NAPI_AUTO_LENGTH, &valuestringOut);
    if (status != napi_ok) {
        /*错误处理*/
     ...
     * result: 指向napi_value的指针，函数执行成功后这个指针将指向新创建的js字符串
     */
    status = napi_create_string_utf8(env, json == nullptr? "": (json->string == nullptr? "": json->string), NAPI_AUTO_LENGTH, &stringOut);
    if (status != napi_ok) {
        /*错误处理*/
        getErrMsg(status, env, extended_error_info, "napi_create_string_utf8", tag);
        return nullptr;
