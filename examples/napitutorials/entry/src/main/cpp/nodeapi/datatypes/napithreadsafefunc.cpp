...
napi_value setThreadsafefunc(napi_env env, napi_callback_info info)
{
    ...
    status = napi_create_threadsafe_function(env, argv[0], nullptr, name, 0, 1,
        nullptr, nullptr, nullptr, CallbackFunction, &g_threadsafeFunction);
    if (status != napi_ok) {
        ...
        if (status == napi_ok && extended_error_info != NULL) {
            ...
            std::string str1 = "Failed to create threadsafe function em = ";
            std::string str2 = ", eec = ";
            std::string str3 = ", ec = "
            OH_LOG_Print(LOG_APP, LOG_ERROR, GLOBAL_RESMGR, TAG, "error_message %{public}s, engine_error_code %{public}d.",
