...
static ArchiveErrorCode MapErrnoToCode(int err) {
    switch (err) {
        case ENOENT: return ArchiveErrorCode::FILE_NOT_FOUND;
        case EACCES:
        case EPERM: return ArchiveErrorCode::FILE_ACCESS_DENIED;
        case ENOSPC:
        ...
        case EROFS: return ArchiveErrorCode::DISK_READ_ONLY;
        case EIO: return ArchiveErrorCode::DISK_IO_ERROR;
        case EEXIST: return ArchiveErrorCode::FILE_ALREADY_EXISTS;
        case EISDIR: return ArchiveErrorCode::FILE_IS_DIRECTORY;
        case ENOTDIR: return ArchiveErrorCode::DIRECTORY_NOT_FOUND;
        case ENAMETOOLONG: return ArchiveErrorCode::FILE_PATH_TOO_LONG;
        case EMFILE:
        case ENFILE: return ArchiveErrorCode::TOO_MANY_OPEN_FILES;
        case ENOMEM: return ArchiveErrorCode::OUT_OF_MEMORY;
        default: return ArchiveErrorCode::UNKNOWN_ERROR;
    }
}
...
// 创建磁盘空间不足错误
static ArchiveError CreateDiskFullError(const std::string& path, uint64_t requiredBytes, 
    ...
    detail << "路径: " << path 
           << "\n需要空间: " << std::fixed << std::setprecision(2) << requiredMB << " MB"
           << "\n可用空间: " << availableMB << " MB"
           << "\n空间不足: " << (requiredMB - availableMB) << " MB";
    
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "❌ 磁盘空间不足！需要=%.2f MB, 可用=%.2f MB",
        requiredMB, availableMB);
    
    return ArchiveError(ArchiveErrorCode::DISK_FULL,
        ErrorMessages::GetMessage(ArchiveErrorCode::DISK_FULL), detail.str());
}

// 检查磁盘空间
ArchiveError ArchiveError::CheckDiskSpace(const std::string& path, uint64_t requiredBytes) {
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "检查磁盘空间: path=%s, 需要=%.2f MB", 
        path.c_str(), requiredBytes / static_cast<double>(BYTES_PER_MB));
...
static ArchiveError HandlePathNotFound(const std::string& path, bool needWrite) {
    ...
    return ArchiveError(ArchiveErrorCode::FILE_NOT_FOUND,
        ErrorMessages::GetMessage(ArchiveErrorCode::FILE_NOT_FOUND), "路径: " + path);
}

// 检查路径访问权限
static ArchiveError CheckPathPermissions(const std::string& path, const struct stat& pathStat, 
                                        bool needWrite) {
    int mode = needWrite ? (R_OK | W_OK) : R_OK;
    if (access(path.c_str(), mode) != 0) {
        int err = errno;
        ArchiveErrorCode code = S_ISDIR(pathStat.st_mode) ? 
            ArchiveErrorCode::DIRECTORY_ACCESS_DENIED : 
            ArchiveErrorCode::FILE_ACCESS_DENIED;
        std::string detail = "路径: " + path + "\n需要权限: " + (needWrite ? "读写" : "只读");
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "❌ 权限不足！path=%s, needWrite=%d",
            path.c_str(), needWrite);
        return ArchiveError(code, ErrorMessages::GetMessage(code), detail);
    }
    return ArchiveError(); // SUCCESS
}
...
ArchiveError ArchiveError::CheckAccess(const std::string& path, bool needWrite) {
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "检查访问权限: path=%s, needWrite=%d", 
        path.c_str(), needWrite);
