...
// 读取文件头数据
static bool ReadFileHeader(const std::string& filePath, unsigned char* header, 
                          size_t headerSize, size_t& bytesRead) {
...
static ArchiveFormat CheckBasicFormats(const unsigned char* header, size_t bytesRead) {
    if (bytesRead >= MAGIC_7Z_SIZE && memcmp(header, MAGIC_7Z, MAGIC_7Z_SIZE) == 0) return ArchiveFormat::SEVENZ;
    ...
    if (bytesRead >= MAGIC_RAR5_SIZE && memcmp(header, MAGIC_RAR5, MAGIC_RAR5_SIZE) == 0) return ArchiveFormat::RAR5;
    if (bytesRead >= MAGIC_RAR_SIZE && memcmp(header, MAGIC_RAR, MAGIC_RAR_SIZE) == 0) return ArchiveFormat::RAR;
    if (bytesRead >= MAGIC_GZIP_SIZE && memcmp(header, MAGIC_GZIP, MAGIC_GZIP_SIZE) == 0) return ArchiveFormat::GZIP;
    if (bytesRead >= MAGIC_BZIP2_SIZE && memcmp(header, MAGIC_BZIP2, MAGIC_BZIP2_SIZE) == 0)
        return ArchiveFormat::BZIP2;
    if (bytesRead >= MAGIC_XZ_SIZE && memcmp(header, MAGIC_XZ, MAGIC_XZ_SIZE) == 0) return ArchiveFormat::XZ;
    if (bytesRead >= MAGIC_LZMA_SIZE && header[0] == 0x5D && header[1] == 0x00 && header[2] == 0x00) {
        return ArchiveFormat::LZMA;
    }
    if (bytesRead >= MAGIC_CAB_SIZE && memcmp(header, MAGIC_CAB, MAGIC_CAB_SIZE) == 0) return ArchiveFormat::CAB;
    if (bytesRead >= MAGIC_WIM_SIZE && memcmp(header, MAGIC_WIM, MAGIC_WIM_SIZE) == 0) return ArchiveFormat::WIM;
...
static ArchiveFormat CheckIsoFormat(const std::string& filePath) {
    ...
    if (!isoFile.good()) return ArchiveFormat::UNKNOWN;
...
ArchiveFormat FormatDetector::DetectBySignature(const std::string& filePath)
{
    ...
    if (format != ArchiveFormat::UNKNOWN) return format;
...
ArchiveFormat FormatDetector::DetectByExtension(const std::string& filePath)
{
    ...
    if (ext == "zip" || ext == "jar" || ext == "apk") return ArchiveFormat::ZIP;
    ...
    if (ext == "lzma86") return ArchiveFormat::LZMA86;
    if (ext == "tar") return ArchiveFormat::TAR;
    if (ext == "iso" || ext == "img") return ArchiveFormat::ISO;
    if (ext == "cab") return ArchiveFormat::CAB;
    if (ext == "wim" || ext == "swm" || ext == "esd") return ArchiveFormat::WIM;
    
    // 检查复合扩展名 (.tar.gz, .tar.bz2, etc.)
    if (ext == "tgz" || ext == "tpz") return ArchiveFormat::GZIP;
    if (ext == "tbz" || ext == "tbz2") return ArchiveFormat::BZIP2;
...
std::string FormatDetector::GetFormatName(ArchiveFormat format)
{
    switch (format) {
        ...
        case ArchiveFormat::LZMA86: return "LZMA86";
