...
// 读取输入文件数据
static bool ReadInputFile(const std::string& inputFile, std::vector<Byte>& data, 
                         size_t& fileSize, std::string* error) {
    std::ifstream inFile(inputFile, std::ios::binary | std::ios::ate);
    if (!inFile.good()) {
        if (error) *error = "无法打开输入文件: " + inputFile;
        return false;
    }
...
// 执行LZMA压缩
static bool ExecuteLzmaCompress(const std::vector<Byte>& inputData,
                               std::vector<Byte>& outputData, SizeT& destLen,
    ...
    int res = LzmaEncode(outputData.data() + LZMA_PROPS_SIZE, &destLen,
        inputData.data(), fileSize, &props, outputData.data(), &propsSize,
        0, nullptr, &g_Alloc, &g_Alloc);
    
    if (res != SZ_OK) {
        if (error) *error = "LZMA 压缩失败，错误码: " + std::to_string(res);
        return false;
    }
    return true;
}
...
// 写入压缩输出文件
static bool WriteCompressedFile(const std::string& outputFile, uint64_t origSize,
                                const std::vector<Byte>& data, SizeT dataLen, 
                                std::string* error) {
    std::ofstream outFile(outputFile, std::ios::binary);
    if (!outFile.good()) {
        if (error) *error = "无法创建输出文件: " + outputFile;
        return false;
    }
...
bool LzmaUtils::CompressFile(
    const std::string& inputFile,
    const std::string& outputFile,
    int level,
    LzmaProgressCallback callback,
    std::string* error)
{
    std::vector<Byte> inputData;
    size_t fileSize = 0;
    if (!ReadInputFile(inputFile, inputData, fileSize, error)) return false;
    
    if (callback) callback(0, fileSize);
...
// 读取压缩文件头和数据
static bool ReadCompressedFile(const std::string& inputFile, uint64_t& origSize,
                               std::vector<Byte>& compressedData, std::string* error) {
    std::ifstream inFile(inputFile, std::ios::binary | std::ios::ate);
    if (!inFile.good()) {
        if (error) *error = "无法打开输入文件: " + inputFile;
        return false;
...
// 执行LZMA解压
static bool ExecuteLzmaDecode(const std::vector<Byte>& compressedData, 
                              std::vector<Byte>& outputData, uint64_t origSize,
                              SizeT& destLen, std::string* error) {
    destLen = origSize;
    SizeT srcLen = compressedData.size() - LZMA_PROPS_SIZE;
    ELzmaStatus status;
    
    int res = LzmaDecode(outputData.data(), &destLen,
        compressedData.data() + LZMA_PROPS_SIZE, &srcLen,
        compressedData.data(), LZMA_PROPS_SIZE,
        LZMA_FINISH_END, &status, &g_Alloc);
    
    if (res != SZ_OK) {
        if (error) *error = "LZMA 解压失败，错误码: " + std::to_string(res);
        return false;
    }
    return true;
}
...
// 写入解压输出文件
static bool WriteDecompressedFile(const std::string& outputFile, 
                                  const std::vector<Byte>& data, SizeT dataLen,
                                  std::string* error) {
    std::ofstream outFile(outputFile, std::ios::binary);
    if (!outFile.good()) {
    ...
    std::string* error)
{
    ...
    if (!ReadCompressedFile(inputFile, origSize, compressedData, error)) return false;
    
    if (callback) callback(0, origSize);
    ...
    if (callback) callback(origSize, origSize);
