...
static std::wstring Utf8ToWstring(const std::string& utf8Str) {
    ...
    while (i < utf8Str.size()) {
        ...
        if ((c & UTF8_1BYTE_MASK) == 0) {
            ...
            if (i + INDEX_OFFSET_NEXT >= utf8Str.size()) break;
            ...
            if (i + INDEX_OFFSET_TWO >= utf8Str.size()) break;
            ...
            if (i + INDEX_OFFSET_THREE >= utf8Str.size()) break;
...
private:
    ...
    public:
        ...
        STDMETHOD(QueryInterface)(REFIID iid, void** outObject) {
            return _parent->QueryInterface(iid, outObject);
        }
...
public:
    CArchiveUpdateCallback(const std::vector<CompressFileItem>* files, 
        ...
        , _processedSize(0)
        , _maxProcessedSize(0)  // 初始化最大进度值
        , _currentIndex(0)
        , _isFinalized(false)  // 初始化为未完成
...
private:
    ...
    void HandleNormalProgress(uint64_t currentValue, uint64_t maxDataProgress) {
        ...
        if (currentValue >= _totalSize * PERCENT_95 / PERCENT_100) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
                "[Progress] Data phase capped at %d%%: raw=%llu, display=%llu / %llu", 
                PERCENT_95, (unsigned long long)currentValue, (unsigned long long)_processedSize, 
    ...
    void HandleFinalizeProgress(uint64_t currentValue, uint64_t maxDataProgress) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
            "[Progress] Finalize: %llu / %llu (%.1f%%) [mapped from %llu]", 
            (unsigned long long)_processedSize, (unsigned long long)_totalSize,
            _totalSize > 0 ? (_processedSize * PERCENT_100 / _totalSize) : 0.0,
...
public:
    STDMETHOD(SetCompleted)(const UInt64* completeValue) {
        ...
        std::string currentFile = (_currentIndex < _files->size()) 
            ? (*_files)[_currentIndex].archivePath : "";
...
private:
    ...
    time_t GetFileStatTime(const std::string& path, int timeType) {
        ...
        if (timeType == FILE_TIME_MTIME) return st.st_mtime;
        if (timeType == FILE_TIME_ATIME) return st.st_atime;
        return st.st_ctime;
    }
...
public:
    STDMETHOD(GetProperty)(UInt32 index, PROPID propID, PROPVARIANT* value) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
            "  GetProperty[%u] propID=%d → vt=%d (success)", index, propID, value->vt);
        return S_OK;
    }
    ...
    STDMETHOD(GetStream)(UInt32 index, ISequentialInStream** inStream) {
        ...
        if (!fileStream->Open(item.sourcePath.c_str())) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG,
                "    → Failed to open file stream: %s", item.sourcePath.c_str());
            delete fileStream;
            return E_FAIL;
        }
    ...
    STDMETHOD(SetOperationResult)(Int32 operationResult) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "  SetOperationResult: result=%d (%s)", 
            operationResult,
...
{
    switch (format) {
        case CompressFormat::SEVENZ: return "7z";
        case CompressFormat::ZIP: return "Zip";
        default: return "Unknown";
    }
}

bool ArchiveCompressor::CompressFile(
    ...
    const CompressOptions& options)
{
    ...
    if (!fs::exists(inputFile)) {
        ArchiveError err(ArchiveErrorCode::FILE_NOT_FOUND, 
            ErrorMessages::GetMessage(ArchiveErrorCode::FILE_NOT_FOUND),
            "文件路径: " + inputFile);
        if (options.error) *options.error = err.GetFullMessage();
        if (options.archiveError) *options.archiveError = err;
    ...
    if (!accessErr.IsSuccess()) {
        if (options.error) *options.error = accessErr.GetFullMessage();
        if (options.archiveError) *options.archiveError = accessErr;
    ...
    try {
        ...
        if (!diskErr.IsSuccess()) {
            if (options.error) *options.error = diskErr.GetFullMessage();
            if (options.archiveError) *options.archiveError = diskErr;
    ...
    if (!writeErr.IsSuccess()) {
        if (options.error) *options.error = writeErr.GetFullMessage();
        if (options.archiveError) *options.archiveError = writeErr;
...
bool ArchiveCompressor::CompressFiles(
    ...
    const CompressOptions& options)
{
    ...
    if (files.empty()) {
        ArchiveError err(ArchiveErrorCode::COMPRESS_NO_INPUT_FILES,
            ErrorMessages::GetMessage(ArchiveErrorCode::COMPRESS_NO_INPUT_FILES),
            "文件列表为空");
        if (options.error) *options.error = err.GetFullMessage();
        if (options.archiveError) *options.archiveError = err;
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "%s", err.GetFullMessage().c_str());
...
static bool ValidateDirectory(const std::string& inputDir, const CompressOptions& options) {
    ...
    if (!S_ISDIR(st.st_mode)) {
        ArchiveError err(ArchiveErrorCode::FILE_IS_DIRECTORY,
            "路径不是目录", "路径: " + inputDir);
        if (options.error) *options.error = err.GetFullMessage();
        if (options.archiveError) *options.archiveError = err;
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "%s", err.GetFullMessage().c_str());
...
// 辅助函数：扫描并收集目录文件
static bool CollectDirectoryFiles(
...
// 辅助函数：检查磁盘空间并验证输出权限
static bool CheckSpaceAndPermissions(
    const std::vector<CompressFileItem>& files,
    const std::string& outputArchive,
    const CompressOptions& options)
{
...
bool ArchiveCompressor::CompressDirectory(
    const std::string& inputDir,
    const std::string& outputArchive,
    const CompressOptions& options)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "=== CompressDirectory START ===");
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "Input: %s, Output: %s",
        inputDir.c_str(), outputArchive.c_str());
    ...
    std::vector<CompressFileItem>& files)
{
    ...
    if (!dir) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "无法打开目录: %s (errno=%d)", currentPath.c_str(), errno);
        return false;
    }
    struct dirent* entry;
...
bool ArchiveCompressor::ScanDirectory(
    const std::string& dirPath,
    const std::string& basePath,
    std::vector<CompressFileItem>& files,
    std::string* error)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "ScanDirectory: dirPath=%s, basePath=%s", 
        dirPath.c_str(), basePath.c_str());
    
    // 检查目录是否存在（使用stat而不是filesystem）
    struct stat st;
    if (stat(dirPath.c_str(), &st) != 0) {
        if (error) *error = "目录不存在或无法访问: " + dirPath;
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "stat failed for %s, errno=%d", dirPath.c_str(), errno);
        return false;
    }
    
    if (!S_ISDIR(st.st_mode)) {
        if (error) *error = "路径不是目录: " + dirPath;
...
static IOutArchive* CreateOutArchive(
    CompressFormat format,
    const CompressOptions& options)
{
    const GUID* clsid = (format == CompressFormat::SEVENZ) ? 
        &CLSID_CFormat7z : &CLSID_CFormatZip;
...
// 设置压缩属性（静态辅助函数）
static bool SetCompressionProperties(
    ...
    const CompressOptions& options)
{
    ...
    if (options.format == CompressFormat::ZIP) {
        ...
        values[0].vt = VT_UI4;
        values[0].ulVal = (options.compressionLevel > DEFAULT_COMPRESSION_LEVEL) ?
            DEFAULT_COMPRESSION_LEVEL : options.compressionLevel;
...
// 创建空压缩包（静态辅助函数）
static bool CreateEmptyArchive(
    const std::string& outputArchive,
    const CompressOptions& options,
    const std::vector<CompressFileItem>& files)
{
    ...
    if (!outStream->Open(outputArchive.c_str())) {
        ...
        if (options.error) *options.error = arcErr.GetFullMessage();
        if (options.archiveError) *options.archiveError = arcErr;
...
// 处理压缩错误
static void HandleCompressionError(HRESULT result, const std::string& outputArchive,
                                   const CompressOptions& options) {
    ...
    std::ostringstream detail;
    detail << "HRESULT: 0x" << std::hex << std::setfill('0') << std::setw(8) 
           << (unsigned int)result << " (" << errName << ")\n输出文件: " << outputArchive;
    
    if (options.format == CompressFormat::SEVENZ && result == HRESULT_E_NOTIMPL) {
        detail << "\n\n⚠️ 提示：7z编码器不可用，请使用ZIP格式";
        OH_LOG_Print(LOG_APP, LOG_WARN, LOG_DOMAIN, LOG_TAG, 
            "7z encoder not available, please use ZIP format instead");
    }
...
// 执行压缩操作（静态辅助函数）
static bool ExecuteCompression(
    IOutArchive* outArchive,
    const std::string& outputArchive,
    const std::vector<CompressFileItem>& files,
    const CompressOptions& options)
{
    ...
    CArchiveUpdateCallback* updateCallback = 
        new CArchiveUpdateCallback(&files, "", totalSize, options.callback);
    
    HRESULT result = outArchive->UpdateItems(
        (ISequentialOutStream*)outStream, files.size(), updateCallback);
...
// 使用p7zip标准接口压缩
bool ArchiveCompressor::CompressWithP7zip(
    const std::vector<CompressFileItem>& files,
    const std::string& outputArchive,
    const CompressOptions& options)
{
