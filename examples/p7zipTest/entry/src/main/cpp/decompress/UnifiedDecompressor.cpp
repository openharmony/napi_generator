...
bool UnifiedDecompressor::Decompress(const std::string &inputFile, const std::string &outputPath,
    ...
    if (!accessErr.IsSuccess()) {
        if (error)
            *error = accessErr.GetFullMessage();
        if (archiveError)
            *archiveError = accessErr;
...
// 检查磁盘空间
static bool CheckDiskSpace(const std::string &outputPath, uint64_t requiredSize, bool useRealSize, std::string *error,
                           ArchiveError *archiveError) {
    if (requiredSize == 0) {
        return true;
    }
    ...
    if (!diskErr.IsSuccess()) {
        if (error)
            *error = diskErr.GetFullMessage();
        if (archiveError)
            *archiveError = diskErr;
...
bool UnifiedDecompressor::DecompressFormat(const std::string &inputFile, const std::string &outputPath,
                                           ArchiveFormat format, DecompressProgressCallback callback,
    ...
    if (!writeErr.IsSuccess()) {
        if (error)
            *error = writeErr.GetFullMessage();
        if (archiveError)
            *archiveError = writeErr;
...
bool UnifiedDecompressor::DecompressLZMA(const std::string &input, const std::string &output,
                                         DecompressProgressCallback callback, std::string *error) {
    // 使用现有的 LzmaUtils
    LzmaProgressCallback legacyCallback = nullptr;
    if (callback) {
        legacyCallback = [callback](uint64_t processed, uint64_t total) { callback(processed, total, ""); };
    }
...
static bool HandleUnimplementedFormat(const std::string &formatName, std::string *error) {
    if (error)
        *error = formatName + " 解压需要进一步实现";
    return false;
}

bool UnifiedDecompressor::DecompressGZIP(const std::string &input, const std::string &output,
                                         DecompressProgressCallback callback, std::string *error) {
    return HandleUnimplementedFormat("GZIP", error);
}

bool UnifiedDecompressor::DecompressBZIP2(const std::string &input, const std::string &output,
                                          DecompressProgressCallback callback, std::string *error) {
    return HandleUnimplementedFormat("BZIP2", error);
}

bool UnifiedDecompressor::DecompressXZ(const std::string &input, const std::string &output,
                                       DecompressProgressCallback callback, std::string *error) {
...
static bool IsStreamFormat(ArchiveFormat format) {
    return format == ArchiveFormat::LZMA || format == ArchiveFormat::LZMA86 || format == ArchiveFormat::GZIP ||
           format == ArchiveFormat::BZIP2 || format == ArchiveFormat::XZ;
}

// 打开归档并获取文件流
static bool OpenArchiveAndStream(const std::string &inputFile, IInArchive *archive, CInFileStream **fileSpec) {
...
// 获取压缩包的真实未压缩大小
bool UnifiedDecompressor::GetUncompressedSize(const std::string &inputFile, ArchiveFormat format,
