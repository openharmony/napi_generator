                                     ...
                                     DecompressProgressCallback callback, DecompressErrorOutput *errorOutput)
{
    ...
    // 根据格式调用对应的解压方法
    return DecompressFormat(inputFile, outputPath, format, callback, errorOutput);
}
// 计算所需磁盘空间
...
// 检查磁盘空间
static bool CheckDiskSpace(const std::string &outputPath, uint64_t requiredSize, bool useRealSize,
...
// 处理不支持的格式
static bool HandleUnsupportedFormat(ArchiveFormat format, DecompressErrorOutput *errorOutput)
{
