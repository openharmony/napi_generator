...
struct AsyncCompressContext {
    ...
    AsyncCompressContext() 
        ...
        , deferred(nullptr)
        , tsfn(nullptr)
        , mode(CompressMode::SINGLE_FILE)
        , compressionLevel(DEFAULT_COMPRESSION_LEVEL)
        , success(false)
        , originalSize(SIZE_ZERO)
        , compressedSize(SIZE_ZERO) 
        , fileCount(INIT_ZERO)
        , taskId(TASKID_ZERO)
        , cancelFlag(nullptr)
        , wasCancelled(false)
...
struct CompressProgressData {
    ...
    CompressProgressData(uint64_t p, uint64_t t, const std::string& f)
        : processed(p), total(t), currentFile(f) {}
};
...
static void ProcessMultipleFiles(AsyncCompressContext* context, const CompressOptions& options) {
    ...
    for (const auto& path : context->inputPaths) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "  [%s] %s -> %s", 
            item.isDirectory ? "DIR " : "FILE", item.sourcePath.c_str(), item.archivePath.c_str());
...
// 处理混合项目压缩（合并内联，精简代码）
static void ProcessMixedItems(AsyncCompressContext* context, 
...
static bool CheckTaskCancelled(AsyncCompressContext* context) {
    if (context->cancelFlag && context->cancelFlag->load()) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
            "任务在开始前已被取消 (ID: %llu)", (unsigned long long)context->taskId);
        context->success = false;
...
static CompressProgressCallback CreateProgressCallback(AsyncCompressContext* context) {
    ...
    return [context](uint64_t processed, uint64_t total, const std::string& currentFile) -> bool {
        if (context->cancelFlag && context->cancelFlag->load()) {
            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
                "检测到取消请求，中断压缩 (任务ID: %llu)", (unsigned long long)context->taskId);
            context->wasCancelled = true;
            return false;
        }
...
static void ExecuteCompressByMode(AsyncCompressContext* context, 
                                  const CompressOptions& options, std::string* error) {
    switch (context->mode) {
...
static void CompressExecute(napi_env env, void* data)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
        "开始异步压缩 -> %s (格式: %s, 模式: %d, 任务ID: %llu)", 
        context->outputPath.c_str(), context->format.c_str(),
        (int)context->mode, (unsigned long long)context->taskId);
    ...
    std::string error;
    CompressOptions options(format, context->compressionLevel, progressCallback, 
        &error, &context->archiveError);
...
static void CleanupTask(AsyncCompressContext* context) {
    if (context->taskId > TASKID_ZERO) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
            "任务已清理 (ID: %llu)", (unsigned long long)context->taskId);
    }
...
// 生成结果消息和错误码
static void GenerateResultMessage(AsyncCompressContext* context, 
...
// 创建基础结果对象
static void CreateBasicResult(napi_env env, napi_value result, 
                              AsyncCompressContext* context,
...
// 添加成功结果的统计信息
static void AddSuccessStatistics(napi_env env, napi_value result, 
                                 AsyncCompressContext* context) {
...
// 解析输入路径数组
static bool ParseInputPaths(napi_env env, napi_value array, 
                            AsyncCompressContext* context) {
    bool isArray = false;
...
// 创建线程安全函数
static bool CreateThreadSafeFunction(napi_env env, napi_value progressCallback,
                                     AsyncCompressContext* context) {
    ...
    napi_status status = napi_create_threadsafe_function(
        ...
        [](napi_env env, napi_value js_callback, void* context, void* data) {
            ...
            if (env != nullptr && js_callback != nullptr) {
                ...
                if (percentage > PERCENT_100) percentage = PERCENT_100;
                napi_create_int32(env, percentage, &percentage_val);
                napi_create_string_utf8(env, progressData->currentFile.c_str(), 
                    NAPI_AUTO_LENGTH, &currentFile_val);
                ...
                napi_call_function(env, undefined, js_callback, NAPI_CALLBACK_ARGS_ONE,
                    &progress_obj, &callback_result);
            }
            delete progressData;
        },
        &context->tsfn
...
static void RegisterTask(AsyncCompressContext* context) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "创建压缩任务 (ID: %llu)", (unsigned long long)context->taskId);
}

// 设置异步上下文参数（合并GetStringParam，减少函数调用开销）
static bool SetupContextParams(napi_env env, napi_value* args, 
                               AsyncCompressContext* context) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, 
        "CompressAsync: 接收到 %zu 个输入路径", context->inputPaths.size());
    ...
    napi_get_value_string_utf8(env, args[INDEX_OFFSET_NEXT], output_buf.data(),
        output_len + VECTOR_BUFFER_OFFSET, &output_len);
    ...
    napi_get_value_string_utf8(env, args[INDEX_OFFSET_TWO], format_buf.data(),
        format_len + VECTOR_BUFFER_OFFSET, &format_len);
...
static bool CreateAsyncWorkTask(napi_env env, AsyncCompressContext* context) {
    ...
    napi_status status = napi_create_async_work(
        env, nullptr, resource_name_work,
        CompressExecute, CompressComplete,
