...
struct ProgressData {
    ...
    ProgressData(uint64_t p, uint64_t t, const std::string& f, const std::string& fmt)
        ...
        if (bracketPos != STRING_NOT_FOUND && slashPos != STRING_NOT_FOUND && 
            closeBracketPos != STRING_NOT_FOUND && bracketPos < slashPos && slashPos < closeBracketPos) {
            try {
                filesCompleted = std::stoi(f.substr(bracketPos + INDEX_OFFSET_NEXT,
                    slashPos - bracketPos - INDEX_OFFSET_NEXT));
                totalFiles = std::stoi(f.substr(slashPos + INDEX_OFFSET_NEXT,
                    closeBracketPos - slashPos - INDEX_OFFSET_NEXT));
            } catch (...) {}
        }
    }
};
...
static bool ValidateInputFile(AsyncDecompressData* asyncData) {
    ...
    if (!testFile.good()) {
        int err = errno;
        asyncData->success = false;
        asyncData->error = "文件无法读取: " + std::string(strerror(err)) + 
...
static DecompressProgressCallback CreateProgressCallback(AsyncDecompressData* asyncData) {
    if (asyncData->tsfn == nullptr) return nullptr;
...
static void ExecuteDecompress(napi_env env, void* data) {
    ...
    if (CheckTaskCancellation(asyncData)) return;
    if (!ValidateInputFile(asyncData)) return;
    
    DecompressProgressCallback progressCallback = CreateProgressCallback(asyncData);
    asyncData->success = UnifiedDecompressor::Decompress(
        asyncData->inputPath.c_str(),
        asyncData->outputPath.c_str(),
        progressCallback,
        &asyncData->error,
        &asyncData->archiveError
    );
...
static void CreateProgressObject(napi_env env, ProgressData* progData, napi_value progress_obj) {
    ...
    if (actualProcessed > actualTotal) actualProcessed = actualTotal;
    ...
    if (actualTotal > SIZE_ZERO) {
        if (actualProcessed >= actualTotal) {
            ...
            if (percentage == INIT_ZERO && actualProcessed > SIZE_ZERO) percentage = INDEX_OFFSET_NEXT;
    ...
    std::string fileInfo = progData->currentFile.empty() 
        ? ("解压 " + progData->formatName + " 中...") 
        : progData->currentFile;
...
napi_value DecompressFileAsync(napi_env env, napi_callback_info info) {
    ...
    napi_create_async_work(env, nullptr, resource_name, ExecuteDecompress, 
        CompleteDecompress, asyncData, &asyncData->asyncWork);
