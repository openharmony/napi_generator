...
本文主要介绍API扫描工具，它可以扫描三方库中包含OpenHarmony源码不包含的接口或.h头文件，并输出result.txt文档。开发者移植三方库到OpenHarmony源码中，若三方库中包含一些OpenHarmony中不存在的接口或.h头文件，便会增加移植难度。此时可使用API扫描工具，提前预知风险接口或.h头文件，降低移植难度，提高开发效率。目前工具支持可执行文件、VS Code插件、IntelliJ插件三种入口。
