...
void XNapiTool::DefineFunction(const char *funcName, napi_callback callback)
{
    napi_property_descriptor descriptor[] = {
        {funcName, 0, callback, 0, 0, 0, napi_default, 0}};

    napi_status result_status = napi_define_properties(env_, exports_, 1, descriptor);
    CC_ASSERT(result_status == napi_ok);
}

void XNapiTool::DefineClass(const char *className, napi_callback constructorFunc, std::map<const char *,std::map<const char *,napi_callback>> &valueList, std::map<const char *, napi_callback> &funcList)
{
    ...
    result_status = napi_set_named_property(env_, exports_, className, tmpClass);
    CC_ASSERT(result_status == napi_ok);
}
