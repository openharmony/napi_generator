...
function checkType2(p, type, name, inParamName, funcValue, param, modifiers) {
...
function checkType3(p, type, name, inParamName, funcValue, param, modifiers) {
...
function paramGenerateArray(p, funcValue, param) {
    ...
    if (type.substring(type.length - 2) === '[]') {
        ...
        checkType2(p, type, name, inParamName, funcValue, param, modifiers);
        ...
        checkType3(p, type, name, inParamName, funcValue, param, modifiers);
    } else {
        NapiLog.logError('The current version do not support to this param to generate :',
            name, 'type :', type);
    }
    return;
}
...
function paramGenerateMap(funcValue, param, p) {
    ...
    if (mapType[1] !== undefined && mapType[2] === undefined) {
        ...
        if (mapType[2] === 'string') {
            ...
            mapTypeString = 'std::map<std::string, ' + mapType[2] + '>';
