...
/**
 * 有符号 8 位整型数组，按 ECMAScript %TypedArray%（Uint8Array 特化）语义实现：
...
public class Uint8Array implements IntArrayView {
    ...
    public Integer set(int[] src, int offset) {
        ...
        if (offset + src.length > length) {
            throw new RangeError("Offset out of range");}
        for (int i = 0; i < src.length; i++) {
            buffer.setInt8(byteOffset + (offset + i) * BYTES_PER_ELEMENT, toUint8(src[i]));}
        return null;}
    ...
    private static String mapDigits(String body, String digits) {
        ...
        for (int i = 0; i < body.length(); i++) {
            ...
            if (c >= '0' && c <= '9') {
                sb.append(digits.charAt(c - '0'));} else {
                sb.append(c);}
        }
        return sb.toString();}
    ...
    public Uint8Array subarray(int begin, int end) {
        ...
        if (subLen < 0) {
            subLen = 0;}
        return new Uint8Array(buffer, byteOffset + from * BYTES_PER_ELEMENT, subLen);}
    ...
    /** 构造 Uint8Array（元素逐一 ToUint8 转换），对应 of 语义。 */
    ...
    public Integer set(java.util.List<Integer> src, int offset) {
        ...
        if (offset + src.size() > length) {
            throw new RangeError("Offset out of range");}
        for (int i = 0; i < src.size(); i++) {
            buffer.setInt8(byteOffset + (offset + i) * BYTES_PER_ELEMENT, toUint8(src.get(i)));}
        return null;}
    ...
    public interface Int16DoubleReducer {
        double apply(double prev, double curr, int index, Uint8Array array);}
