...
/**
 * 有符号 8 位整型数组，按 ECMAScript %TypedArray%（Uint8ClampedArray 特化）语义实现：
 ...
 * @since 2026-08-26
 */
public class Uint8ClampedArray implements IntArrayView {
    ...
    public Uint8ClampedArray(IntArrayView src) {
        this(src.length());
    ...
    public Uint8ClampedArray(ArrayBuffer buf) {
        this(buf, 0, checkedElements(buf));}

    public Uint8ClampedArray(ArrayBuffer buf, int byteOffset) {
        this(buf, byteOffset, (checkedElements(buf) * BYTES_PER_ELEMENT - byteOffset) / BYTES_PER_ELEMENT);}

    public Uint8ClampedArray(ArrayBuffer buf, double byteOffset) {
        this(buf, (int) byteOffset);}

    public Uint8ClampedArray(ArrayBuffer buf, double byteOffset, double length) {
        this(buf, (int) byteOffset, (int) length);}

    public Uint8ClampedArray(ArrayBuffer buf, int byteOffset, int length) {
    ...
    public Integer set(int index, double value) {
        if (index < 0 || index >= length) {
            throw new RangeError("Index out of range");}
        buffer.setInt8(byteOffset + index * BYTES_PER_ELEMENT, toUint8Clamp(value));
        return null;}
    ...
    public Integer set(int[] src, int offset) {
        ...
        if (offset + src.length > length) {
            throw new RangeError("Offset out of range");}
        for (int i = 0; i < src.length; i++) {
            buffer.setInt8(byteOffset + (offset + i) * BYTES_PER_ELEMENT, toUint8Clamp(src[i]));}
        return null;}
    ...
    public Uint8ClampedArray map(Uint8ClampedArrayMapper cb) {
        ...
        for (int i = 0; i < length; i++) {
            mapped[i] = toUint8Clamp(cb.apply(get(i), i, this));}
        return new Uint8ClampedArray(mapped);}
    ...
    private static String formatIntl(int value, String locales, IntlOptions opts) {
        ...
        if ("scientific".equals(notation) || "engineering".equals(notation)) {
            body = scientific(amount, "engineering".equals(notation));} else if ("compact".equals(notation)) {
            body = compact(amount, compactDisplay);} else {
            int fracDigits = 0;
            if (minSig > 0) {
    ...
    private static String attachCurrency(String body, String cur, String display, String lang) {
        ...
        if ("name".equals(display)) {
            String name;
            if ("USD".equals(cur)) {
                name = "US dollars";} else if ("GBP".equals(cur)) {
                name = "British pounds";} else if ("EUR".equals(cur)) {
                name = "euros";} else if ("JPY".equals(cur)) {
                name = "Japanese yen";} else {
                name = cur;}
            return body + " " + name;}
        String symbol;
        if ("USD".equals(cur)) {
            symbol = "$";} else if ("EUR".equals(cur)) {
            symbol = "\u20AC";} else if ("GBP".equals(cur)) {
            symbol = "\u00A3";} else if ("JPY".equals(cur)) {
            symbol = lang.startsWith("ja") || lang.startsWith("zh") ? "\uFFE5" : "\u00A5";} else {
            symbol = cur;}
    ...
    public Uint8ClampedArray subarray(int begin, int end) {
        ...
        if (subLen < 0) {
            subLen = 0;}
        return new Uint8ClampedArray(buffer, byteOffset + from * BYTES_PER_ELEMENT, subLen);}
...
public Uint8ClampedArray copyWithin(double target, double start, double end) {
    ...
    public Uint8ClampedArray toSorted() {
        ...
        for (int i = 0; i < length; i++) {
            copy[i] = get(i);}
        Uint8ClampedArray sorted = new Uint8ClampedArray(copy);
        sorted.sort();
        return sorted;}
    ...
    public Integer set(java.util.List<Integer> src, int offset) {
        ...
        for (int i = 0; i < src.size(); i++) {
            buffer.setInt8(byteOffset + (offset + i) * BYTES_PER_ELEMENT, toUint8Clamp(src.get(i)));}
        return null;}
