...
public class Int16ArrayFindTest01 extends BasTest {
    ...
    Integer result = arr.find((value, index, array) -> {
        return index == 3 && value == 28;
        });
    assertEqualInt(28, result);
    }
    ...
    Integer result = arr.find((value, index, array) -> {
        return index > 0 && array.get(index - 1) == 4;
        });
    assertEqualInt(8, result);
    }
    ...
    Integer result = arr.find((value) -> {
        externalArr[0] = new Int16Array(new int[] {0});
        return value == 20;
    });
    assertEqualInt(20, result);
    }
    ...
    try {
    arr.find((value) -> {
    return BasTest.throwTestError("predicate error");
        });
    fail();
    } catch (Error e) {
    ...
    callIdx[0]++;
    if (callIdx[0] == 2) {
    return BasTest.throwTestError("second call error");
    }
    return false;
        });
    fail();
    ...
    if (index == 2) {
    return BasTest.throwTestError("middle error");
    }
    return false;
        });
    fail();
    ...
    return BasTest.throwTestError("last element error");
    }
    return false;
        });
    fail();
