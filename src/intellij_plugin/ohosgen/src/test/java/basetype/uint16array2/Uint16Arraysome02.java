...
public class Uint16Arraysome02 extends BasTest {
    ...
    boolean outerResult = arr.some((e) -> {
        if (e > 1) { boolean inner = arr.some((x) -> x == 3);
        return inner;
        } return false;
    });
    ...
    {
    Uint16Array arr = new Uint16Array(new int[] {1, 2, 3});
    boolean result = arr.some((e) -> { try { if (e == 2) { throw new Error("caught");
    } return false;
    } catch (Error inner) { assertEqual("Error", inner.getClass().getSimpleName());
    return true;
    } });
    assertTrue(result);
    }
    ...
    boolean r = a.some((n, i) -> {
        if (i == 0) { alias.set(2, 33);
        } return n == 33;
    });
    assertTrue(r);
    }
