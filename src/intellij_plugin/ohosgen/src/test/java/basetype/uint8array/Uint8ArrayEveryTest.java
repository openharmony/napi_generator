...
public class Uint8ArrayEveryTest extends BasTest {
    ...
    Uint8Array arr = new Uint8Array();
    boolean result = arr.every((v) -> { called[0] = true; return true;});
    ...
    Uint8Array arr = Uint8Array.of(10, 20, 30);
    int[] lastIndex = {-1};
    arr.every((v, idx) -> { lastIndex[0] = idx; return true;});
    ...
    Uint8Array arr = Uint8Array.of(1, 2, 3);
    Uint8Array[] ref = {null};
    arr.every((v, idx, a) -> { ref[0] = a; return true;});
    ...
    Uint8Array arr = Uint8Array.of(1, 2, 3);
    int[] len = {-1};
    arr.every((v, idx, a) -> { len[0] = a.length(); return true;});
    assertEqual(3, len[0]);}
    /**
    ...
    arr.every((v, idx, a) -> { isUint8[0] = BasTest.instanceOf(a, Uint8Array.class); return true;});
    ...
    int[] count = {0};
    Uint8Array arr = Uint8Array.of(0, 1, 2);
    arr.every((v) -> { count[0]++; return v > 0;});
    ...
    int[] count = {0};
    Uint8Array arr = Uint8Array.of(1, 2, 3);
    arr.every((v) -> { count[0]++; return true;});
    ...
    everyThrowCount = 0;
    try {
    arr.every(Uint8ArrayEveryTest::everyThrowAndCount);} catch (Error e) {
    assertEqual("Error", e.getClass().getSimpleName());};
