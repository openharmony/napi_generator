...
public class Uint8ClampedArrayFindTest extends BasTest {
    ...
    arr.find((v, i, a) -> {
    if (i == 0) {
        return BasTest.throwTestError("idx0");
    }
    return v == 2;
        });
    fail();
    ...
    arr.find((v, i, a) -> {
    if (i == 1) {
        return BasTest.throwTestError("idx1");
    }
    return false;
        });
    fail();
    ...
    assertEqualInt(0, order.get(0));
    assertEqualInt(1, order.get(1));
    assertEqualInt(2, order.get(2));
    assertEqualInt(3, order.get(3));
    }
