...
| **ohrecord** | **snapshot_record** 屏幕录制 **MP4**：路径/SELinux/编译/设备准备、录制与拉取、**hilog** 与 **.so** 特征校验；完整修改清单与排查见 **ohproj/SKILL.md §10** | `ohrecord/ohrecord.py`（子命令如 `paths`、`build`、`prep-device`、`record`、`pull`、`verify-remote-mp4`、`hilog-capture`、`targets` 等） |
...
| **ohrecord** | 「准备 snapshot_record 目录与标签」「录一段屏并拉回 MP4」「查录屏相关 hilog」 | `python3 src/skills/ohrecord/ohrecord.py prep-device` / `record -f /data/test/media/x.mp4 …` / `pull` / `hilog-capture`（完整见 `SKILL.md`） |
