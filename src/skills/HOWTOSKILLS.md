...
> 7. 配置环境变量（路径换成你本机实际目录）  
>    - **构建**：`export HOS_CLT_PATH=<Command Line Tools 根目录>`；`export OHOS_SDK_PATH=<ohos-sdk 或 DevEco SDK 根目录>`  
>    - **Linux / macOS**：上述用 `export`；**Windows PowerShell** 示例：`$env:HOS_CLT_PATH="..."`；`$env:OHOS_SDK_PATH="..."`  
>    - **仅 HAP 签名**（`hapbuild.py sign`）：还需 **`OHOS_HAPSIGNER_RESULT`** 指向含 `OpenHarmony.p12`、`rootCA.cer`、`subCA.cer` 及 profile **`.pem`** 的目录（常为 OpenHarmony 源码 `developtools/hapsigner/autosign/result`，或自备合并目录）；签名解析 SDK 时 **`OHOS_SDK_PATH` 与 `DEVECO_SDK_HOME` 二选一或均可**。详见下文 **§4.4** 与 **ohhap/SKILL.md**。
...
| **ohhap** | HAP 主包 / ohosTest 构建、**release/debug 签名**（项目下 **`autosign/`** 拷贝证书与工具后 **`hap-sign-tool`**）、**清除签名**、环境检查；**Windows / Linux 同一脚本**，SDK 下 **`linux/toolchains/lib`** 与 **`<API>/toolchains/lib`** 自动适配；**p12 旧别名** 可 **`keytool`** 清理（见 **ohhap/SKILL.md**） | `ohhap/hapbuild.py` |
...
| **ohdoc** | 从 **`任务7.2：NAPI测试用例模板.docx`** 与 **`*.test.ts`** 生成 **`napitest…docx`**（**`napi-test-doc`**）；或与 **测试用例列表 CSV** 生成 **`csvtest…docx`**（**`csv-test-doc --csv …`**） | `ohdoc/ohdoc.py napi-test-doc` / **`csv-test-doc`**（**`pip install -r ohdoc/requirements.txt`**） |
...
| **ohxtsstatic** | **ArkTS `use static` + Hypium XTS 一体化**（非简单合并）：**分层优先级**（SDK→SKILL→`categories`→`common`）+ **五步流水线**（§〇 归类→A–D→工具链）；**ohxtsstatic/arkui-static-xts-generator/** 为第 1.5 / 第 2 层资料层；**hapbuild**、**ohhdc** deploy-test/**hilog**/**faultlog**；**compile_error_hints**；**ohxtsflow.py**；**静态工程编译**须使用 **`$HOS_CLT_PATH/hvigor-static/bin/hvigorw.js`**：执行 **`hapbuild`/`ohxtsflow build-all` 前** **`export OHOS_USE_HVIGOR_STATIC=1`**（或 **`OHOS_HVIGORW_JS`** 指向 **hvigorw.js** 的完整路径），见 **ohxtsstatic/SKILL.md** | `ohxtsstatic/ohxtsflow.py`；**ohxtsstatic/SKILL.md**（v1.3+） |
...
- **ohhap**：**目前主要验证 6.0 / API 20 类工程**（`build-profile` 中 SDK 版本可为 **`6.0.0(20)`** 或**数字 `20`**）。**构建**：`HOS_CLT_PATH`、`OHOS_SDK_PATH`。**签名**：另需 **`OHOS_HAPSIGNER_RESULT`**（及可选 **`OHOS_HAPSIGNER_AUTOSIGN`**）；**`OHOS_SDK_PATH` 或 `DEVECO_SDK_HOME`**；**`java`**，建议 **`keytool`** 与 `java` 同属一套 JDK/JRE（用于清理工作副本 p12 内不兼容的 **`oh-app1-key-v1`**）。**Windows** 上 DevEco SDK 常见为 **`…\ohossdk\<API>\toolchains\lib\hap-sign-tool.jar`**；**Linux** 常见为 **`$OHOS_SDK_PATH/linux/toolchains/lib/`**。未设置 **`OHOS_HAPSIGNER_RESULT`** 时会尝试 **`~/ohos/…/hapsigner/autosign/result`**（Windows 对应 **`%USERPROFILE%\ohos\…`**）。完整步骤与产物路径见 **ohhap/SKILL.md**。
- **ohdoc**：需 **`pip install -r src/skills/ohdoc/requirements.txt`**（**python-docx**）；**`ohdoc/template/`** 下须自备 **NAPI 测试用例** **`.docx`**（脚本默认取体积最大者）；**`*.docx` / `*.csv` 不入库**，克隆后自行放入模板与 CSV。**`napi-test-doc`**：**`--test-dir`** 合并时默认 **`ohdoc/napitestYYYYMMDD.docx`**。**`csv-test-doc`**：默认 **`ohdoc/csvtestYYYYMMDD.docx`**，**`--exec-date`** 默认 **2026/04/10**；CSV 填表关键列见 **§4.5**（**用例标识**=**用例编号**，**所属攻关任务**=**投屏分享**，**用例追溯**=**功能模块**）。目标 **`.docx` 若被 Word 打开会 `PermissionError`**，请关闭后重试或换 **`--out`**。无 **`test(...)`** 的 **`*.test.ts`** 会跳过并打警告。详见 **ohdoc/SKILL.md**、**§4.5**。
...
- **ohxtsstatic**：编排依赖 **ohhap/ohhdc**；规范与排障以本目录 **SKILL.md**、**compile_error_hints.md** 为准；**ArkUI 接口范式** 见 **`arkui-static-xts-generator/`**（**须从 [arkUISkill（GitCode）](https://gitcode.com/qq_44921954/arkUISkill) 自行下载放置**，见 **`arkui-static-xts-generator/README.md`**；可选对比见仓库 **`_compare_upload/`**）；**静态用例工程**编译前设置 **`OHOS_USE_HVIGOR_STATIC=1`**，使 **`hapbuild`** 使用 **`hvigor-static`**（见 **SKILL.md**）；编译/用例「自动修复」由 Agent 对照上述文档迭代，非单一脚本保证。

**环境与路径（常用）**

- HAP / 部分工具：`HOS_CLT_PATH`、`OHOS_SDK_PATH`（见本文档开头）；**HAP 签名**另见 **`OHOS_HAPSIGNER_RESULT`**、**`DEVECO_SDK_HOME`**（§4.4）。
...
| **ohhap** | 「编译 EmptyProj46R / NativeProj46R」「只打 ohosTest」「对未签名 HAP 做 release 签名」「清除 autosign」 | 仓库根：`python3 src/skills/ohhap/hapbuild.py build src/skills/ohhap/EmptyProj46R`；**签名前**设置 **`OHOS_HAPSIGNER_RESULT`**（及 **`OHOS_SDK_PATH` 或 `DEVECO_SDK_HOME`**），再 `… hapbuild.py sign <项目> [release|debug]`；`build-test` / `clean-sign` 同上。Windows 可将 **`python3`** 换成 **`python`**。环境变量示例见 **§4.4**。 |
...
| **ohdoc** | 「按模板从单个 ts 生成 Word」「**某目录全部 `*.test.ts` 合并成一篇**」「**从 CSV 列表生成 csvtest{日期}.docx**」 | **`pip install -r src/skills/ohdoc/requirements.txt`** 后：`… napi-test-doc --test-file …` / **`… napi-test-doc --test-dir …/parse`** / **`… csv-test-doc --csv …/11314873测试用例列表_*.csv`**（**`--out`**；**`csv-test-doc`** 默认执行日期 **2026/04/10**） |
...
| **ohxtsstatic** | 「一体化 static + Hypium」「§〇 路由+检查点」「SDK+明细+docs」「build-all/deploy-test」 | `python3 src/skills/ohxtsstatic/ohxtsflow.py` 同上；**SKILL.md**（融合模型）+ **compile_error_hints** + **arkui-static-xts-generator/**（外置下载） |
...
- **签名额外变量**：
  - **`OHOS_HAPSIGNER_RESULT`**：含 **`OpenHarmony.p12`**、**`rootCA.cer`**、**`subCA.cer`**、**`OpenHarmonyProfileRelease.pem`**（release）或 Debug 对应 pem 的目录。源码标准布局为 **`…/developtools/hapsigner/autosign/result`**；若证书与 **`Unsgned*ProfileTemplate.json` 在同一目录**，只设此项即可（**ohhap/SKILL.md** 称「合并 layout」）。
  - **`OHOS_HAPSIGNER_AUTOSIGN`**（可选）：证书在 **`result`**、模板在上级 **`autosign`** 且自动推断失败时再设。
  - **`OHOS_SDK_PATH` 或 `DEVECO_SDK_HOME`**：用于定位 **`hap-sign-tool.jar`**（先按 **`build-profile`** 的 API 试 **`<SDK>/<API>/toolchains/lib`**，再试 **`linux/toolchains/lib`**）。
- **Java**：**`java`** 必在 PATH；**`keytool`** 建议可用（与 **`java`** 同 JDK/JRE），以便删除工作副本 p12 内旧的 **`oh-app1-key-v1`**（与 OS 无关的证书历史问题）。
- **示例（Linux）**：

```bash
export HOS_CLT_PATH=/path/to/command-line-tools
export OHOS_SDK_PATH=/path/to/ohos-sdk
export OHOS_HAPSIGNER_RESULT=/path/to/openharmony/developtools/hapsigner/autosign/result
python3 src/skills/ohhap/hapbuild.py build src/skills/ohhap/EmptyProj46R
python3 src/skills/ohhap/hapbuild.py sign src/skills/ohhap/EmptyProj46R release
```

- **示例（Windows PowerShell）**：

```powershell
$env:HOS_CLT_PATH = "D:\Program Files\Huawei\command-line-tools"
$env:OHOS_SDK_PATH = "D:\Program Files\Huawei\ohossdk"
$env:OHOS_HAPSIGNER_RESULT = "D:\path\to\hapsigner\autosign\result"
python src\skills\ohhap\hapbuild.py build src\skills\ohhap\EmptyProj46R
python src\skills\ohhap\hapbuild.py sign src\skills\ohhap\EmptyProj46R release
```

- **产物**：已签名 HAP 一般在 **`entry/build/default/outputs/default/*-signed.hap`**，与 **`*-unsigned.hap`** 同目录；中间文件在 **`<项目>/autosign/`**。

更细的 8 步流程、错误排查与 **`build-profile`** 中 Windows 路径签名字段处理，见 **`ohhap/SKILL.md`** 与 **`hapbuild.py`** 文件头说明。

### 4.5 ohdoc：NAPI 测试用例 Word 文档（`napi-test-doc` / `csv-test-doc`）

- **入口**：在 **napi_generator 仓库根**：`python src/skills/ohdoc/ohdoc.py napi-test-doc …` 或 **`… csv-test-doc --csv <路径.csv> …`**（Windows 可用 **`python`**；Linux/macOS 常用 **`python3`**）。
- **依赖**：`pip install -r src/skills/ohdoc/requirements.txt`（**python-docx**）。
- **模板**：**`src/skills/ohdoc/template/`** 下 **「任务7.2」类 NAPI 测试用例** **`.docx`**（**本地自备**，仓库不提交 **`.docx`/`.csv`**）；脚本默认选该目录下**体积最大**的 **`.docx`**（与「设计文档」模板区分），也可用 **`--template`** 指定。
- **`napi-test-doc` 单文件**：`--test-file <路径>` → 输出一篇 **`napitest{YYYYMMDD}.docx`**（默认目录 **`src/skills/ohdoc`**，或 **`--out`** / **`--out-dir`**）。
- **`napi-test-doc` 目录合并**：`--test-dir <目录>` → 该目录全部 **`*.test.ts`**（排序后）**合并为单篇** docx；文首列全部源路径；每个文件首次出现前插入 **`【xxx.test.ts】`**；**任务1…任务N** 全文连续编号；某文件无 **`test(...)`** 则跳过并 **⚠ 警告**。
- **`napi-test-doc` 填表要点**：解析 **suite**、首个业务 **import**（跳过 **assert/vscode**）、**test** 名、**//** 注释、**parsec.*** / **parsets.***、**assert.**；版式为标题段 **`addnext`** 表格、表间 spacer（与 **`ohdoc.py`** 一致）。
