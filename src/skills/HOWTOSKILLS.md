...
- **ohdoc**：需 **`pip install -r src/skills/ohdoc/requirements.txt`**（**python-docx**）；模板 **`ohdoc/template/`** 下 **NAPI测试用例** **`.docx`**（脚本默认取体积最大者）须存在。**`napi-test-doc`**：**`--test-dir`** 合并时默认 **`ohdoc/napitestYYYYMMDD.docx`**。**`csv-test-doc`**：默认 **`ohdoc/csvtestYYYYMMDD.docx`**，**`--exec-date`** 默认 **2026/04/10**；CSV 填表关键列见 **§4.5**（**用例标识**=**用例编号**，**所属攻关任务**=**投屏分享**，**用例追溯**=**功能模块**）。目标 **`.docx` 若被 Word 打开会 `PermissionError`**，请关闭后重试或换 **`--out`**。无 **`test(...)`** 的 **`*.test.ts`** 会跳过并打警告。详见 **ohdoc/SKILL.md**、**§4.5**。
...
- **模板**：**`src/skills/ohdoc/template/`** 下 **「任务7.2」类 NAPI 测试用例** **`.docx`**；脚本默认选该目录下**体积最大**的 **`.docx`**（与「设计文档」模板区分），也可用 **`--template`** 指定。
