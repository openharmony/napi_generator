...
| **ohxtsstatic** | **ArkTS `use static` + Hypium XTS 一体化**（非简单合并）：**分层权威**（SDK→SKILL→`categories`→`common`）+ **五步流水线**（§〇 归类→A–D→工具链）；**ohxtsstatic/arkui-static-xts-generator/** 为 L1.5/L2 资料层；**hapbuild**、**ohhdc** deploy-test/**hilog**/**faultlog**；**compile_error_hints**；**ohxtsflow.py**；**静态工程编译**须使用 **`$HOS_CLT_PATH/hvigor-static/bin/hvigorw.js`**：执行 **`hapbuild`/`ohxtsflow build-all` 前** **`export OHOS_USE_HVIGOR_STATIC=1`**（或 **`OHOS_HVIGORW_JS`** 绝对路径），见 **ohxtsstatic/SKILL.md** | `ohxtsstatic/ohxtsflow.py`；**ohxtsstatic/SKILL.md**（v1.3+） |
