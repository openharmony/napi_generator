...
def collect_mdk_apis(src_root: Path, comp_path: str) -> list[dict]:
    """在组件目录下查找 *.mdk.json，解析出 API 名称与 first_introduced（api 版本）。返回 [{"name", "first_introduced", "mdk_path"}, ...]。"""
    comp_dir = src_root / comp_path
    if not comp_dir.is_dir():
        return []
    apis: list[dict] = []
    for mdk_path in sorted(comp_dir.rglob("*.mdk.json")):
        try:
            data = json.loads(mdk_path.read_text(encoding="utf-8", errors="ignore"))
        except Exception:
            continue
        if not isinstance(data, list):
            continue
        rel_mdk = mdk_path.relative_to(src_root).as_posix() if mdk_path.is_relative_to(src_root) else mdk_path.name
        for item in data:
...
def run_scan(src_root: Path) -> tuple[list[dict], dict[str, list[dict]], dict]:
    ...
    for b in bundles:
        ...
        mdk_apis = collect_mdk_apis(src_root, rel_dir)
        comp_row = {
            "name": comp_name,
            "subsystem": subsys,
            "path": rel_dir,
    ...
    mdkapi_total = sum(len(c.get("mdk_apis") or []) for c in components)
    counts = {
...
def write_scan_report(
    ...
    mdkapi_total = counts.get("mdkapi_total", 0)
    ...
    # 子系统排名（按 mdkapi 数量 Top 50）
    subsys_mdkapi = []
    for subsys_name, items in subsystems.items():
        comp_paths = {x["path"] for x in items}
        n_mdkapi = sum(len(c.get("mdk_apis") or []) for c in components if c.get("path") in comp_paths)
        subsys_mdkapi.append((subsys_name, n_mdkapi))
    subsys_mdkapi.sort(key=lambda x: -x[1])
    subsys_mdkapi = subsys_mdkapi[:50]
    lines.extend([
        "",
        "## 子系统排名（按 mdkapi 数量 Top 50）",
        "",
    ...
    for rank, (subsys_name, n_mdk) in enumerate(subsys_mdkapi, 1):
        sn = subsys_name.replace("|", "\\|")
        lines.append(f"| {rank} | {sn} | {n_mdk} |")
    lines.extend([
        "",
    ...
    for c in sorted(components, key=lambda x: (x["subsystem"], x["name"])):
        ...
        for api in c.get("mdk_apis") or []:
            ...
            api_path = str(api.get("mdk_path", "")).replace("|", "\\|")
...
def _build_diff_report(
    ...
    # 新增/删除 mdkapi（表格：mdkapi名称、子系统、组件、api版本、路径）
    set_mdkapi1 = set()
    for c in components1:
        subsys = str(c.get("subsystem") or "")
        comp_name = str(c.get("name") or c.get("path") or "")
        for api in c.get("mdk_apis") or []:
            ...
            if name:
                set_mdkapi1.add((name, subsys, comp_name))
    set_mdkapi2 = set()
    for c in components2:
        subsys = str(c.get("subsystem") or "")
        comp_name = str(c.get("name") or c.get("path") or "")
        for api in c.get("mdk_apis") or []:
            name = str(api.get("name") or "")
            if name:
                set_mdkapi2.add((name, subsys, comp_name))
    added_mdkapi_set = set_mdkapi2 - set_mdkapi1
    removed_mdkapi_set = set_mdkapi1 - set_mdkapi2
    added_mdkapi_rows = []
    for c in components2:
        subsys = str(c.get("subsystem") or "")
        comp_name = str(c.get("name") or c.get("path") or "")
        for api in c.get("mdk_apis") or []:
            name = str(api.get("name") or "")
            if name and (name, subsys, comp_name) in added_mdkapi_set:
                ver = str(api.get("first_introduced") or "")
                path = str(api.get("mdk_path") or "")
                added_mdkapi_rows.append((name, subsys, comp_name, ver, path))
    removed_mdkapi_rows = []
    for c in components1:
        subsys = str(c.get("subsystem") or "")
        comp_name = str(c.get("name") or c.get("path") or "")
        for api in c.get("mdk_apis") or []:
            name = str(api.get("name") or "")
            if name and (name, subsys, comp_name) in removed_mdkapi_set:
                ver = str(api.get("first_introduced") or "")
                path = str(api.get("mdk_path") or "")
                removed_mdkapi_rows.append((name, subsys, comp_name, ver, path))
    added_mdkapi_rows.sort(key=lambda x: (x[0], x[1], x[2]))
    removed_mdkapi_rows.sort(key=lambda x: (x[0], x[1], x[2]))
    lines.extend(["", f"## 新增 mdkapi 列表（{len(added_mdkapi_rows)}）", ""])
    if added_mdkapi_rows:
        lines.append("| mdkapi名称 | 子系统 | 组件 | api版本 | 路径 |")
        lines.append("|------------|--------|------|--------|------|")
        for name, subsys, comp_name, ver, path in added_mdkapi_rows:
        ...
        lines.append("（无）")
    lines.extend(["", f"## 删除 mdkapi 列表（{len(removed_mdkapi_rows)}）", ""])
    if removed_mdkapi_rows:
        lines.append("| mdkapi名称 | 子系统 | 组件 | api版本 | 路径 |")
        lines.append("|------------|--------|------|--------|------|")
        for name, subsys, comp_name, ver, path in removed_mdkapi_rows:
            name_esc = name.replace("|", "\\|")
            subsys_esc = subsys.replace("|", "\\|")
            comp_esc = comp_name.replace("|", "\\|")
            ver_esc = ver.replace("|", "\\|")
