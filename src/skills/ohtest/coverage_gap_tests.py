...
def format_recommendation(module_name: str, module_root: Optional[Path], file_gaps: List[dict], fuzz_targets: List[str]) -> str:
    ...
    lines.append("    python3 src/skills/ohbuild/ohbuild.py build-fuzztest <NewFuzzTest> --gn-args <module>_feature_coverage=true")
    lines.append("  运行 fuzz 并收集覆盖率：")
    lines.append("    python3 src/skills/ohtest/fuzztest.py run -ts <NewFuzzTest> --coverage")
    lines.append("  生成 .gcov 并统计：")
    lines.append("    python3 src/skills/ohtest/coverage_analysis.py run && python3 src/skills/ohtest/coverage_analysis.py analyze")
    lines.append("")
