...
1. **自动跑设备用例**：**`run-static-pipeline`** 或 **`static-device-test`**（或与设备侧 **应用测试** 子流程等效，见 **ohhdc** 的 `static-deploy-test`），使本批次 Hypium 在设备上执行。  
...
2. **执行命令**（代码块）  
   - 完整可复制的 `run-static-pipeline` / `static-deploy-test` / 设备应用测试命令行。  
...
| **`arkui-static-xts-generator/`** | **测试范式资料层（第 1.5 / 第 2 层）**：目录内 **`categories/`**、**`common/`**、**`SKILL.md`** 应与 **[arkUISkill（GitCode）](https://gitcode.com/qq_44921954/arkUISkill)** 仓库中 **`arkui-static-xts-generator`** 保持同步；本仓在 `ohxtsstatic/arkui-static-xts-generator/` 下另附 **`README.md`**（集成说明）。若另有独立下载的生成器压缩包，请与官方对比后再合并，对比方式见 **`/root/aiSkill/_compare_upload/README.md`**。 | P0 |
...
| 层级 | 来源 | 职责 |
|------|------|------|
| **第零层（SDK）** | **SDK `.d.ets`** | 能否编译、签名、类型；**最终以本列为准** |
| **第一层（本 skill）** | 本文 **§一～五**、**`compile_error_hints.md`** | `use static` + Hypium/UiTest **硬约束**、多机布局、隔离、日志、路由 |
| **第 1.5 层（范式归类）** | **`arkui-static-xts-generator/categories/*.md`** | **测什么、怎么验收**：核心思路、检查点、验证方法（按接口 **§〇** 选篇） |
| **第二层（书写细则）** | **`arkui-static-xts-generator/common/`**（`import` / `ets_rules` / `test_rules`） | **书写细则**：显式导入参考、页面与用例骨架、**`@tc.*` JSDoc**；与 **第零层 / 第一层** 冲突时 **服从 SDK 与本 skill** |
...
| 5 扩展能力 API | `5-扩展能力API/`（与上游目录对应时请以 generator 仓库为准） | `5.1-页面绑定型系统能力.md`、`5.2-独立型系统能力.md`、`5.3-基础控制器.md`、`5.4-导航类API.md`、`5.5-FrameNode.md` |
...
**静态 XTS**：**`ohhdc static-deploy-test`** 对设备下发 **`-m entry`**，且 **`-s timeout`** 置于 **`-s unittest`**（Runner 多为类名 **`OpenHarmonyTestRunner`**）之前，与上游文档一致。若 Runner 必须使用文件路径而非类名，可通过 **`OHOS_A​A_TEST_UNITTEST_RUNNER`** 或 **`--unittest-runner`** 指定。
...
长时间 Hypium 全量：**先** `export OHOS_A​A_TEST_WALL_SEC=7200`（秒）再跑 **`static-device-test` / `run-static-pipeline`**，避免本机过早终止设备应用测试子进程（与 `--timeout` 设备毫秒不是同一含义，见 **ohhdc/SKILL.md**）。
