...
**静态 XTS**：**`ohhdc static-deploy-test`** 对设备下发 **`-m entry`**，且 **`-s timeout`** 置于 **`-s unittest`**（Runner 多为类名 **`OpenHarmonyTestRunner`**）之前，与上游文档一致。若 Runner 必须使用文件路径而非类名，可通过 **`OHOS_AA_TEST_UNITTEST_RUNNER`** 或 **`--unittest-runner`** 指定。
