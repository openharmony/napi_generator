...
// function genStubRet(srcName: string, parcelName: string, vType: string) {
//   let matchs = match('(std::)?vector<([\x21-\x7e]+)[ ]?>', vType);
//   if (matchs) {
//       // vector类型变量包装成parcel data
//       let rawType = getReg(vType, matchs.regs[2]);
...
// function genStubParam(parcelName: string, destObj: DestObj) {
//   let matchs = match('(std::)?vector<([\x21-\x7e]+)[ ]?>', destObj.type);
//   if (matchs) {
//       // 从parcel data中读取vector类型变量tb4
//       let rawType = getReg(destObj.type, matchs.regs[2]);
//       let parcelVecType = getParcelType(rawType);
//       let rVecFunc = getTransferContent(parcelVecType, 1);
//       if (rVecFunc === '') {
//           return '';
//       }
//       return format('%s.%s(&(%s));', parcelName, rVecFunc, parcelName);
...
//  */
// function genVectorWrite(vectorName: string, parcelName: string, vecType: string, matchs: any) {
//     let rawType = getReg(vecType, matchs.regs[2]);
//     let parcelType = getParcelType(rawType);
//     let wFunc = VECTOR_W_MAP.get(parcelType);
//     if (!wFunc) {
//         return '';
//     }
//     return format('%s.%s(%s);', parcelName, wFunc, vectorName);
// }
...
//  */
// function genVectorRead(parcelName: string, vectorName: string, vecType: string, matchs: any) {
//     let rawType = getReg(vecType, matchs.regs[2]);
//     let parcelType = getParcelType(rawType);
...
//         return '';
//     }
//     return format('%s.%s(&(%s));', parcelName, rFunc, vectorName);
// }
...
//     let result = format('%s = %s.%s();', destObj.name, parcelName, rFunc);
//     return result;
// }
