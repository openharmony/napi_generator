...
const PICK_CONFIGURE = vscode.l10n.t('Use configure script for compilation.');
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const ohcrosscompile = vscode.commands.registerCommand('extension.ohcrosscompile', async (uri) => {
      ...
      if (uri && uri.path) {
        ...
        if (platform === "win32") {
          ...
          canConfigure = fs.existsSync(thirdPartyPath.concat("/configure"));
        }        
        
        // 如果检测到CMakeLists.txt或makefile或configure脚本，则可以继续
        if (canCmake || canMake || canConfigure) {  
          ...
          // 获取编译方式是make还是cmake还是configure
          if (configContent.settings.compileTool !== undefined && (configContent.settings.compileTool === "make" || configContent.settings.compileTool === "cmake" || configContent.settings.compileTool === "configure")) {
            //如果配置文件中已经存储编译方式，则获得
            compileTool = configContent.settings.compileTool;
          } else if (canMake ? (canCmake || canConfigure) : (canCmake && canConfigure)) {
            // 若有两个或两个以上的可选编译方式，则询问用户，并存储结果
            let toolPickItems: {
              label: string;
              description: string;
            }[] = [];
            ...
            // 若自动判断出只可以使用configure
            compileTool = "configure";
            configContent.settings.compileTool = "configure";
            fs.writeFileSync(configPath, JSON.stringify(configContent, null, 4), 'utf8');
          }
          ...
          // 用户所选文件夹不含CMakeLists.ext、Makefile、configure
    ...
    const ohGenerator = vscode.commands.registerCommand('extension.ohGenerator', async () => {
      ...
      let welcomeCtrl = new WelcomeCtrl(uri);
      welcomeCtrl.init();
      // // The code you place here will be executed every time your command is executed
