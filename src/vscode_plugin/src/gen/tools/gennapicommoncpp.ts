...
export function doGenCommonCppFile(rootInfo: GenInfo, fileContent: string) {
  try {
    fileContent = replaceAll(fileContent, '[fileName]', rootInfo.fileName);
  } catch (error) {
    let errmsg = 'generate napi common.cpp fileContent error: ' + JSON.stringify(error);
...
export function genCommonCppFile(rootInfo: GenInfo, filePath: string, fileContent: string) {
  fileContent = doGenCommonCppFile(rootInfo, fileContent);
