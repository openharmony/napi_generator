...
suite('Stability_DTS2CPP_COMPAT_FUNC_Part03', function() {
  ...
  test('dts2cpp_compat_func_0003', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat38.ts', `function compat38(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat38", "dts2cpp_compat_func_0003 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
      ...
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:boolean)=>void", "dts2cpp_compat_func_0005_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0007', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat38.ts', `declare namespace ns { export function compat38(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0013', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat39.ts', `function compat39(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat39", "dts2cpp_compat_func_0013 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:number,p1:number)=>number", "dts2cpp_compat_func_0015_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0017', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat39.ts', `declare namespace ns { export function compat39(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0023', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat40.ts', `function compat40(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat40", "dts2cpp_compat_func_0023 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:number,p1:boolean)=>number", "dts2cpp_compat_func_0025_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0027', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat40.ts', `declare namespace ns { export function compat40(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0033', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat41.ts', `function compat41(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat41", "dts2cpp_compat_func_0033 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:string,p1:string)=>number", "dts2cpp_compat_func_0035_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0037', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat41.ts', `declare namespace ns { export function compat41(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0043', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat42.ts', `function compat42(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat42", "dts2cpp_compat_func_0043 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:boolean,p1:number)=>number", "dts2cpp_compat_func_0045_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0047', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat42.ts', `declare namespace ns { export function compat42(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0053', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat43.ts', `function compat43(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat43", "dts2cpp_compat_func_0053 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:boolean,p1:boolean)=>number", "dts2cpp_compat_func_0055_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0057', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat43.ts', `declare namespace ns { export function compat43(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0063', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat44.ts', `function compat44(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat44", "dts2cpp_compat_func_0063 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:number,p1:number)=>string", "dts2cpp_compat_func_0065_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0067', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat44.ts', `declare namespace ns { export function compat44(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0073', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat45.ts', `function compat45(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat45", "dts2cpp_compat_func_0073 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:number,p1:boolean)=>string", "dts2cpp_compat_func_0075_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0077', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat45.ts', `declare namespace ns { export function compat45(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0083', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat46.ts', `function compat46(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat46", "dts2cpp_compat_func_0083 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:string,p1:string)=>string", "dts2cpp_compat_func_0085_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0087', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat46.ts', `declare namespace ns { export function compat46(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
