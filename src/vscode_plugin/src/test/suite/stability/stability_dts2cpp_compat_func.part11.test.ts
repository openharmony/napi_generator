...
suite('Stability_DTS2CPP_COMPAT_FUNC_Part11', function() {
  ...
  test('dts2cpp_compat_func_0005', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat123.ts', `function compat123(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat123", "dts2cpp_compat_func_0005 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
      ...
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:any,p1:number)=>object", "dts2cpp_compat_func_0007_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0009', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat123.ts', `declare namespace ns { export function compat123(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0015', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat124.ts', `function compat124(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat124", "dts2cpp_compat_func_0015 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:any,p1:string)=>object", "dts2cpp_compat_func_0017_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0019', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat124.ts', `declare namespace ns { export function compat124(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0025', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat125.ts', `function compat125(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat125", "dts2cpp_compat_func_0025 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:any,p1:boolean)=>object", "dts2cpp_compat_func_0027_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0029', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat125.ts', `declare namespace ns { export function compat125(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0035', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat126.ts', `function compat126(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat126", "dts2cpp_compat_func_0035 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:object,p1:any)=>object", "dts2cpp_compat_func_0037_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0039', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat126.ts', `declare namespace ns { export function compat126(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0045', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat127.ts', `function compat127(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat127", "dts2cpp_compat_func_0045 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:object,p1:object)=>object", "dts2cpp_compat_func_0047_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0049', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat127.ts', `declare namespace ns { export function compat127(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0055', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat128.ts', `function compat128(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat128", "dts2cpp_compat_func_0055 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:object,p1:number)=>object", "dts2cpp_compat_func_0057_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0059', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat128.ts', `declare namespace ns { export function compat128(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0065', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat129.ts', `function compat129(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat129", "dts2cpp_compat_func_0065 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:object,p1:string)=>object", "dts2cpp_compat_func_0067_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0069', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat129.ts', `declare namespace ns { export function compat129(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0075', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat130.ts', `function compat130(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat130", "dts2cpp_compat_func_0075 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:object,p1:boolean)=>object", "dts2cpp_compat_func_0077_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0079', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat130.ts', `declare namespace ns { export function compat130(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
  ...
  test('dts2cpp_compat_func_0085', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat131.ts', `function compat131(a: ): void {}`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.funcs && converted.funcs.length >= 1) {
            assert.strictEqual(converted.funcs[0].name, "compat131", "dts2cpp_compat_func_0085 func name");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          assert.ok(converted === undefined || typeof converted === 'object');
          if (converted.types && converted.types.length >= 1) {
            assert.strictEqual(converted.types[0].alias, "(p0:number,p1:any)=>object", "dts2cpp_compat_func_0087_alias");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
      ...
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          if (converted.funcs && converted.funcs.length >= 1 &&
            ...
            assert.strictEqual(converted.funcs[0].parameters[0].type,
              "std::function<std::any(double, std::any)>",
              "dts2cpp_compat_func_0088_param convert output");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
  ...
  test('dts2cpp_compat_func_0089', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const parsed = doParseTs('compat131.ts', `declare namespace ns { export function compat131(a: Numbr): void; }`);
        assert.ok(parsed === undefined || typeof parsed === 'object');
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          const converted = transParseObj(parsed);
          assert.ok(converted === undefined || typeof converted === 'object');
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        if (parsed !== undefined && parsed !== null && typeof parsed === 'object') {
          ...
          if (converted.funcs && converted.funcs.length >= 1 &&
              converted.funcs[0].parameters && converted.funcs[0].parameters.length >= 1) {
            assert.strictEqual(converted.funcs[0].parameters[0].type, "strng", "dts2cpp_compat_func_0091 param type");
          }
        }
      });
      assert.ok(finished, 'compat task must finish without process crash');
