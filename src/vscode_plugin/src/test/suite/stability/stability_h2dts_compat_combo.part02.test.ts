...
suite('Stability_H2DTS_COMPAT_COMBO_Part02', function() {
  this.timeout(600000);
  vscode.window.showInformationMessage('Start Stability_H2DTS_COMPAT_COMBO_Part02.');
  ...
  test('h2dts_compat_combo_0001', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(doubl p);`);
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0001 param type");
      });
  ...
  test('h2dts_compat_combo_0002', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0002 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0003', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0003 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0004', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0004 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0005', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat207(std::weak_ptr<uint64_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0005_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0006', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(bad_type_207 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_207", "h2dts_compat_combo_0006 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0007', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat207(std::weak_ptr<uint64_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0007_param convert output non-empty");
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0007_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0008', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E207 { A, B }; void f(E207 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0008 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0008 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0008 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0009', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<uint64_t>", "h2dts_compat_combo_0009 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0010', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0010 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0011', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0011 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0012', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0012 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0013', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0013 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0014', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0014 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0015', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat208(std::weak_ptr<int8_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0015_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0016', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(bad_type_208 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_208", "h2dts_compat_combo_0016 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0017', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat208(std::weak_ptr<int8_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0017_param convert output non-empty");
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0017_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0018', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E208 { A, B }; void f(E208 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0018 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0018 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0018 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0019', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<int8_t>", "h2dts_compat_combo_0019 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0020', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0020 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0021', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0021 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0022', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0022 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0023', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0023 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0024', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0024 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0025', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat209(std::weak_ptr<int16_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0025_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0026', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(bad_type_209 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_209", "h2dts_compat_combo_0026 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0027', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat209(std::weak_ptr<int16_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0027_param convert output non-empty");
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0027_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0028', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E209 { A, B }; void f(E209 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0028 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0028 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0028 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0029', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<int16_t>", "h2dts_compat_combo_0029 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0030', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0030 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0031', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0031 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0032', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0032 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0033', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0033 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0034', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0034 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0035', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat210(std::weak_ptr<int32_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0035_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0036', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(bad_type_210 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_210", "h2dts_compat_combo_0036 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0037', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat210(std::weak_ptr<int32_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0037_param convert output non-empty");
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0037_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0038', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E210 { A, B }; void f(E210 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0038 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0038 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0038 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0039', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<int32_t>", "h2dts_compat_combo_0039 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0040', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0040 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0041', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0041 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0042', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0042 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0043', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0043 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0044', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0044 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0045', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat211(std::weak_ptr<int64_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0045_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0046', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(bad_type_211 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_211", "h2dts_compat_combo_0046 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0047', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat211(std::weak_ptr<int64_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0047_param convert output non-empty");
        assert.strictEqual(converted, "number", "h2dts_compat_combo_0047_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0048', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E211 { A, B }; void f(E211 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0048 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0048 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0048 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0049', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<int64_t>", "h2dts_compat_combo_0049 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0050', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0050 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0051', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0051 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0052', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0052 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0053', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0053 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0054', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0054 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0055', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat212(std::weak_ptr<bool> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(converted, "boolean", "h2dts_compat_combo_0055_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0056', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(bad_type_212 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_212", "h2dts_compat_combo_0056 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0057', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat212(std::weak_ptr<bool> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0057_param convert output non-empty");
        assert.strictEqual(converted, "boolean", "h2dts_compat_combo_0057_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0058', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E212 { A, B }; void f(E212 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0058 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0058 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0058 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0059', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<bool>", "h2dts_compat_combo_0059 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0060', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0060 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0061', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0061 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0062', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0062 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0063', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0063 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0064', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0064 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0065', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat213(std::weak_ptr<char> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0065_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0065_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0066', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(bad_type_213 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_213", "h2dts_compat_combo_0066 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0067', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat213(std::weak_ptr<char> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0067_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0067_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0068', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E213 { A, B }; void f(E213 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0068 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0068 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0068 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0069', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<char>", "h2dts_compat_combo_0069 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0070', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0070 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0071', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0071 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0072', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0072 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0073', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0073 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0074', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0074 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0075', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat214(std::weak_ptr<wchar_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0075_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0075_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0076', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(bad_type_214 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_214", "h2dts_compat_combo_0076 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0077', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat214(std::weak_ptr<wchar_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0077_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0077_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0078', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E214 { A, B }; void f(E214 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0078 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0078 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0078 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0079', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<wchar_t>", "h2dts_compat_combo_0079 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0080', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0080 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0081', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0081 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0082', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0082 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0083', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0083 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0084', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0084 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0085', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat215(std::weak_ptr<char8_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0085_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0085_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0086', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(bad_type_215 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_215", "h2dts_compat_combo_0086 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0087', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat215(std::weak_ptr<char8_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0087_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0087_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0088', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E215 { A, B }; void f(E215 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0088 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0088 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0088 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0089', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<char8_t>", "h2dts_compat_combo_0089 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0090', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0090 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0091', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0091 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0092', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(boool p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "boool", "h2dts_compat_combo_0092 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0093', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(intt p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "intt", "h2dts_compat_combo_0093 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0094', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(flot p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "flot", "h2dts_compat_combo_0094 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0095', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`namespace ns { void compat216(std::weak_ptr<char16_t> p); }`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0095_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0095_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0096', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(bad_type_216 p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "bad_type_216", "h2dts_compat_combo_0096 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0097', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat216(std::weak_ptr<char16_t> p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        const converted = transTskey2Ckey(r[0].parameters[0].type);
        assert.ok(typeof converted === 'string' && converted.length > 0,
          "h2dts_compat_combo_0097_param convert output non-empty");
        assert.strictEqual(converted, "string", "h2dts_compat_combo_0097_param convert output");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0098', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`enum E216 { A, B }; void f(E216 e);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0098 parseFunction must return non-empty array");
        assert.ok(Array.isArray(r) && r.length >= 1,
          "h2dts_compat_combo_0098 must parse function");
        assert.strictEqual(r[0].name, "f", "h2dts_compat_combo_0098 func name");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0099', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        ...
        assert.strictEqual(r[0].parameters[0].type, "std:weak_ptr<char16_t>", "h2dts_compat_combo_0099 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0100', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat217(itn p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "itn", "h2dts_compat_combo_0100 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
  ...
  test('h2dts_compat_combo_0101', function() {
    ...
    try {
      const finished = runCompatSafe(() => {
        const r = parsec.parseFunction(`void compat217(doubl p);`);
        assert.ok(Array.isArray(r) && r.length >= 1,
        ...
        assert.strictEqual(r[0].parameters[0].type, "doubl", "h2dts_compat_combo_0101 param type");
      });
      assert.ok(finished, 'compat task must finish without process crash');
    } catch (err) {
